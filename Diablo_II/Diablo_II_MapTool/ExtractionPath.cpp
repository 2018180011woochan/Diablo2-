...
BEGIN_MESSAGE_MAP(CExtractionPath, CDialog)
	ON_WM_DROPFILES()
	ON_BN_CLICKED(IDC_BUTTON1, &CExtractionPath::OnBnClickedSaveButton)
	ON_BN_CLICKED(IDC_BUTTON5, &CExtractionPath::OnBnClickedLoadButton)
END_MESSAGE_MAP()
...
void CExtractionPath::OnDropFiles(HDROP hDropInfo)
{
	...
	for (auto& pTexPath : m_listTexPath)
	{
		...
		wstrCombine = pTexPath->wstrRelativePath + L"|"
			+ pTexPath->wstrObjectKey + L"|"
			+ pTexPath->wstrStateKey + L"|"
			+ szCount;
...
void CExtractionPath::OnBnClickedSaveButton()
{
	...
	if (!fout.fail())
	{
		for (auto& pTexPath : m_listTexPath)
		{
			fout << pTexPath->wstrRelativePath << L"|"
				<< pTexPath->wstrObjectKey << L"|"
				<< pTexPath->wstrStateKey << L"|"
				<< pTexPath->dwCount << endl;
		}
		fout.close();
	}
...
void CExtractionPath::OnBnClickedLoadButton()
{
	...
	if (!fin.fail())
	{
		...
		while (true)
		{
			...
			if (fin.eof())
				break;
			wstrCombine = wstring(szPath) + L"|"
				+ wstring(szObjectKey) + L"|"
				+ wstring(szStateKey) + L"|"
				+ wstring(szCount);
			m_ListBox.AddString(wstrCombine.c_str());
		}
		fin.close();
