...
HRESULT CObj::Ready_Object()
{
	m_vecObject.reserve(TILEX * TILEY);
	return S_OK;
}

void CObj::Render_Object()
{
	...
	{
		const TEXINFO* pTexInfo = CTexture_Manager::Get_Instance()->Get_TexInfo(L"Object", L"Object", m_vecObject[i]->byDrawID);
...
void CObj::Obj_Insert(const _vec3 & vPos, const BYTE & byDrawID)
{
	if (byDrawID == 99)
		return;
