...
void CStartTerrain::TileChange_Terrain(const _vec3 & vPos, const BYTE & byDrawID, const BYTE & byOption)
{
...
void CStartTerrain::InitTile(const _vec3 & vPos, const BYTE & byDrawID)
{
	int iIndex = Get_TileIndex(vPos);
...
int CStartTerrain::Get_TileIndex(const _vec3 & vPos)
{
...
bool CStartTerrain::IsPicking(const _vec3 & vPos, const int & iIndex)
{
	_vec3 vPoint[4] =
	{
...
void CStartTerrain::Set_Ratio(_matrix & matWorld, float fRatioX, float fRatioY)
{
...
HRESULT CStartTerrain::Ready_Terrain()
{
	TILE* pTile = nullptr;
...
void CStartTerrain::Render_Terrain()
{
	...
	{
		const TEXINFO* pTexInfo = CTexture_Manager::Get_Instance()->Get_TexInfo(L"Texture", L"ConturyTile", m_vecTile[i]->byDrawID);
...
void CStartTerrain::Mini_Render_Terrain()
{
	...
	for (size_t i = 0; i < dwSize; ++i)
	{
		const TEXINFO* pTexInfo = CTexture_Manager::Get_Instance()->Get_TexInfo(L"Terrain", L"ConturyTile", m_vecTile[i]->byDrawID);
		assert(!(nullptr == pTexInfo));
		// 		if (nullptr == pTexInfo)
		// 			return; 
		float fCenterX = float(pTexInfo->tImageInfo.Width >> 1);
